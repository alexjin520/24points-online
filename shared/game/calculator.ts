...
export class Calculator {
  ...
  static getSolutionHint(cards: number[]): string | null {
    ...
    for (const perm of permutations) {
      const hint = this.findSolutionSteps(perm);
      if (hint) {
        return hint;
      }
    }
  ...
  private static findSolutionSteps(nums: number[]): string | null {
    const [a, b, c, d] = nums;
    const ops: Operation[] = ['+', '-', '*', '/'];
    ...
    for (const op1 of ops) {
      for (const op2 of ops) {
        for (const op3 of ops) {
          // Try structure: ((a op1 b) op2 c) op3 d
          const hint = this.tryExpressionWithSteps(a, b, c, d, op1, op2, op3, 'left');
          if (hint) return hint;
          
          // Try structure: (a op1 (b op2 c)) op3 d
          const hint2 = this.tryExpressionWithSteps(a, b, c, d, op1, op2, op3, 'middle-left');
          if (hint2) return hint2;
          
          // Try structure: (a op1 b) op2 (c op3 d)
          const hint3 = this.tryExpressionWithSteps(a, b, c, d, op1, op2, op3, 'balanced');
  ...
  private static tryExpressionWithSteps(
    a: number, b: number, c: number, d: number,
    op1: Operation, op2: Operation, op3: Operation,
    structure: 'left' | 'middle-left' | 'balanced'
  ): string | null {
    try {
      let result: number;
      let steps: string;

      const opSymbol = (op: Operation) => {
        switch (op) {
          case '+': return '+';
          case '-': return '-';
          case '*': return '×';
         ...
         case 'left':
           // ((a op1 b) op2 c) op3 d
           const step1 = this.applyOp(a, b, op1);
           const step2 = this.applyOp(step1, c, op2);
           result = this.applyOp(step2, d, op3);
           steps = `${a} ${opSymbol(op1)} ${b} = ${step1}, ${step1} ${opSymbol(op2)} ${c} = ${step2}, ${step2} ${opSymbol(op3)} ${d} = ${result}`;
           break;
         case 'middle-left':
           // (a op1 (b op2 c)) op3 d
           const substep1 = this.applyOp(b, c, op2);
           const substep2 = this.applyOp(a, substep1, op1);
           result = this.applyOp(substep2, d, op3);
           steps = `${b} ${opSymbol(op2)} ${c} = ${substep1}, ${a} ${opSymbol(op1)} ${substep1} = ${substep2}, ${substep2} ${opSymbol(op3)} ${d} = ${result}`;
           break;
         case 'balanced':
           // (a op1 b) op2 (c op3 d)
           const left = this.applyOp(a, b, op1);
           const right = this.applyOp(c, d, op3);
           result = this.applyOp(left, right, op2);
           steps = `${a} ${opSymbol(op1)} ${b} = ${left}, ${c} ${opSymbol(op3)} ${d} = ${right}, ${left} ${opSymbol(op2)} ${right} = ${result}`;
           break;
         default:
           return null;
       }
